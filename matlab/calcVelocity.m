function [velEvents,t_dist,success] = calcVelocity(allEvents,N,epsilon,mu,REG_ON,SHOW_PLOT,IMAGE_FRAME)
%
...
for i = 1:last % loop trough input data
	...
	if(p == -1)
		...
		if(((IMAGE_FRAME(2)-floor(N/2)>=x) && (x>=ceil(N/2))) && ((IMAGE_FRAME(1)-floor(N/2)>=y) && (y>=ceil(N/2))))
	...
	if(p == 1)
		...
		% check if pixel in imageFrame, depending on window size N
		if(((IMAGE_FRAME(1)-floor(N/2)>=x) && (x>=ceil(N/2))) && ((IMAGE_FRAME(1)-floor(N/2)>=y) && (y>=ceil(N/2))))
			% read out local NxN surface window
